...
//
// ⚠️⚠️ 여기서는 dvh 를 절대 쓰지 말 것. dvh 는 "지금 보이는 높이"라 모바일에서 스크롤로 브라우저 툴바가
// 접혔다 펴질 때마다 값이 변한다 → 리스트/미리보기가 스크롤 중에 늘었다 줄었다 하며 요동친다.
// svh = "툴바가 보이는 상태(가장 작은 뷰포트)" 기준 고정값이라 스크롤 중에도 절대 변하지 않는다.
// 루트(body)만 dvh 를 쓰고(PinkbeanShop), 그 안의 섹션 높이는 전부 svh + px 하한으로 고정한다.
//
// max(비율svh, px하한) → 작은 폰(SE, 667px)에서도 카드가 절대 안 무너진다.
// 비율이 실효 지배값인 점에 주의: SE 에서 66svh≈400px 라 px 하한만 올리면 효과가 없다 → 둘 다 올린다.
// 합이 100svh 를 넘는 만큼 문서 스크롤이 생기고, 다 내리면 리스트+미리보기가 화면을 꽉 채운다.
// 미리보기는 모델 크기를 사용자가 확정 → 건드리지 않는다.
export const MOBILE_H = {
  list: 'max(66svh, 400px)',     // 코디
  search: 'max(70svh, 452px)',   // AI 검색: 검색 입력 행이 하나 더 있다
  content: 'max(68svh, 424px)',  // 코디 정보·염색 / 프리셋(내부 스크롤)
  preview: 'max(38svh, 244px)',
} as const
