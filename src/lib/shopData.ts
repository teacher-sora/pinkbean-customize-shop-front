...
// expr 을 주면 표정도 반영한다(표정 얼굴장식 — fixedExpr 참고). 안 주면 종전대로 'default'.
export function thumbView(gaze: string, expr?: string): { view: ViewOpts; flip: boolean } {
  return {
    view: { ...THUMB_VIEW, action: gaze === 'back' ? 'rope' : 'stand1', expression: expr || THUMB_VIEW.expression },
    flip: gaze === 'right',
  }
}
