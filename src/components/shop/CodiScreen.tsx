...
import { CAT_TO_SLOT, DOT_MOVER_IDS, SLOT_TO_CAT, THUMB_VIEW, animaLayers, fixedExpr, forceMyModel, hasFixedExpr, isColorLineSkin, thumbView } from '@/lib/shopData'
...
// 카드 배경(모델=맨 마네킹 / 내모델=내 착용) 조립 결과. expr/faceMeta 는 표정 얼굴장식 카드가
// 얼굴을 자기 표정으로 다시 그릴 때 쓴다.
type Ctx = { items: AssembleInput[]; key: string; override: Map<string, HTMLCanvasElement>; effs: WornEff[]; expr: string; faceMeta: ItemMeta | null }
const EMPTY_CTX: Ctx = { items: [], key: '', override: new Map(), effs: [], expr: 'default', faceMeta: null }
...
export default function CodiScreen() {
  ...
  // 그런데 mode==='model' 의 ctx 는 base(몸통+머리)뿐이라 승격된 카드가 그릴 배경이 없다
  // → 그 아이템이 리스트에 있으면 '내 모델' 컨텍스트(myCtx)를 하나 더 만들어 그 카드에만 넘긴다.
  const needMy = mode === 'model' && list.some(hasFixedExpr)
  const [ctx, setCtx] = useState<Ctx>(EMPTY_CTX)
  const [myCtx, setMyCtx] = useState<Ctx>(EMPTY_CTX)
  ...
  useEffect(() => {
    ...
    const ids = [...(isSkinCat ? [] : [bodyId, headId]), ...(mode === 'mymodel' || needMy ? myEq.map(([, it]) => it.id) : [])]
    Promise.all(ids.map((id) => loadMeta(id).then((m) => [id, m] as const).catch(() => null))).then(async (res) => {
      ...
      const build = async (worn: [string, ListItem][], expr: string, k: string, isMy: boolean): Promise<Ctx> => {
        ...
        if (worn.length) {
          ...
          override = await buildOverrides(dyeMetas, { palette: s.dyePalette, hsb: s.dyeHsb }, view).catch(() => new Map())
          // 착용 아이템의 이펙트(망토 오라 등)도 카드에 그린다 + 그 이펙트 염색을 override 에 굽는다.
          // 연출 토글이 꺼진 슬롯은 아예 제외 → 카드에서도 안 보인다.
          effs = await collectWornEffects(worn.map(([sl, it]) => ({ slot: sl, id: it.id })), s.pv, s.dyeHsb, override).catch(() => [])
        }
