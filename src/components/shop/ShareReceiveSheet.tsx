...
export default function ShareReceiveSheet() {
  ...
  // 닫힐 때도 전환이 보이도록 값이 사라진 뒤에도 잠깐 더 그린다(프리셋을 고르면 applySharedToPreset 이 바로 비운다).
  const { shown: snap, hidden } = useDialogFade(s.sharedIncoming)
  if (!snap) return null
  ...
  return (
    <div {...mask} className={clsx('pb-dlg-mask', hidden && 'pb-dlg-hidden')}
      style={css('position:fixed; inset:0; z-index:60; display:flex; align-items:center; justify-content:center; padding:16px; background:rgba(42,37,33,0.45);')}>
      <div onClick={(e) => e.stopPropagation()} className={clsx('pb-dlg-panel', hidden && 'pb-dlg-hidden')} style={css(`width:100%; max-width:${mob ? 520 : 720}px; max-height:88vh; background:#fff; border:1px solid #e7ded4; border-radius:16px; display:flex; flex-direction:column; overflow:hidden; box-shadow:0 20px 60px rgba(42,37,33,.22);`)}>
